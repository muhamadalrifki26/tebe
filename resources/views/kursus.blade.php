			...
			<div class="sidebar left">
				...
				<div class="my-account-nav-container">
					...
					<ul class="my-account-nav">
						...
						<li><a href="kursus" class="current"><i class="sl sl-icon-docs"></i> List Kursus</a></li>
			...
			<table class="manage-table responsive-table">
				...
				<tr>
					<td class="title-container">
						...
						<div class="title">
							<h4><a href="{{ url('detailkursus/'.$value->id) }}">{{ $value->nama }}</a></h4>
							<span>{{ $value->alamat }}</span>
							<span class="table-property-price">{{ $value->tarif_peserta }} </span>
						</div>
					</td>
					<td class="expire-date">{{ $value->lokasi }}</td>
					<td class="action">
						<a href="{{ url('/kursusedit/'.$value->id) }}"><i class="fa fa-pencil"></i> Edit</a>
						<a href="#"><i class="fa  fa-eye-slash"></i> Hide</a>
						<a href="{{ url('/kursushapus/'.$value->id) }}" onclick="return confirm('Yakin Hapus ?')" class="delete"><i class="fa fa-remove"></i> Delete</a>
					</td>
				</tr>
				@endforeach
